...
# dd if=/dev/zero of=xx.img bs=1M count=1536
# losetup /dev/loop0 xx.img
# 
...
cleanup() {
	...
	*/loop*)
		kpartx -dv /dev/loop0
		losetup -dd /dev/loop0
		echo "kpartx -dv /dev/loop0 ............"
		;;
	esac
...
partitionSD () {
	...
	*/mmcblk*)
		...
		;;
	*/loop*)
		subdevice="/dev/mapper/loop0p"
		echo "xxxxxxxxxxxxxxx $subdevice"
		;;
	*)
		subdevice="$1"
		;;
...
mountPartitions ()
{
	...
	*/loop*)
		subdevice="/dev/mapper/loop0p"
		echo "yyyyyyyyyyyyyyy $subdevice"
		;;
	*)
		subdevice="$1"
		;;
